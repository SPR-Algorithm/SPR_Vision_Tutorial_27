...
> **关于 `demo.mp4`**：视频部分暂时不做。逐帧识别本身不难，但要做成闭环还涉及整车建模观测与坐标系转换，属于后面的内容。这一段只做「单张图 → 装甲板四点」。
...
- `armor_detect.cpp` 里 `detectArmor()` 的输入是 `cv::Mat`、输出是四个点，**不依赖任何框架**。以后要做成 ROS2 节点，直接把这个函数搬进去，外面套一层「订阅 → 转换 → 调用 → 发布」就行，算法本身一行不用改。
