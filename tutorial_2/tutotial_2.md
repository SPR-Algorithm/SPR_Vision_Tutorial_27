...
> 本阶段暂时只做前两项：**面向对象** 与 **OpenCV 装甲板识别**。
> ROS2 部分（框架设计、识别节点封装、整车建模与坐标系转换、视频流处理）
> 牵涉的内容较多，放到后面单独做，教案里暂不展开。
...
> 原提纲的作业是「基于 ROS2 实现一个传统装甲板识别节点」。ROS2 部分暂时不做，
> 本阶段先把**不依赖任何框架的识别算法本身**做出来：用 OpenCV 传统方法
> 从一张装甲板图片中找出装甲板的**四个角点**。等框架部分补上之后，
> 这份算法代码可以直接被节点调用，不需要改动。

用 OpenCV 传统图像处理方法，从装甲板图片中识别出装甲板，输出它的**四个角点**
（左上 / 右上 / 右下 / 左下），并把识别结果画在图上。

### 3.2 要求

1. **面向对象**：识别逻辑封装成一个类（如 `ArmorDetector`），对外只暴露一个 `detect()` 接口，`main()` 只负责读图和显示。
2. **传统方法**：灰度 → 二值化 → 形态学 → 轮廓 → 灯条筛选 → 灯条配对 → 四点输出，不允许用神经网络。
3. **参数集中**：阈值、长宽比范围这些写在类里的 `Params` 结构体或常量里，不要散落在函数中间。
4. **输出**：终端打印四个角点坐标；`result.png` 上画出装甲板外框，四个角点标成 P0~P3。
5. **鲁棒性**：能正确处理图里的干扰——白色数字、线束、背景高光。
6. **讲解**：能画出识别流水线，说明每个几何约束的物理含义。

### 3.3 评分表（满分 100）

| 维度         | 分值 | 评分要点                                        |
| ------------ | ---- | ----------------------------------------------- |
| 面向对象设计 | 20   | 识别逻辑封装成类；接口清晰；不依赖全局变量      |
| 流水线正确性 | 25   | 灰度 / 二值化 / 形态学 / 轮廓各步顺序与参数合理 |
| 识别正确性   | 35   | 四点落在两条灯条上；对干扰物有鲁棒性            |
| 代码规范     | 10   | 参数集中、命名清晰、`-Wall -Wextra` 无警告      |
| 原理讲解     | 10   | 能讲清每个几何约束为什么这么定                  |
...
1. `struct` 和 `class` 有什么区别？为什么要给成员变量加 `private`？
2. 为什么识别逻辑要封装成类，而不是写一长串函数？
3. `cv::Mat b = a;` 之后修改 `b`，`a` 会变吗？为什么？
4. 为什么二值化用 OTSU 而不是写死一个阈值？
5. `findContours` 之前为什么要先做形态学开运算？
6. 灯条筛选为什么用「长宽比」而不是「面积」？两者各挡掉什么？
7. 灯条配对用了哪些几何约束？各自的物理含义是什么？
8. 为什么只比「宽高比接近 2.5」会选错？加大小权重解决了什么问题？
9. 输出的四个点为什么要固定顺序？
10. 如果光照变暗，哪一步会最先失效？怎么改？
