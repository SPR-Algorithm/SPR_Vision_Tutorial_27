...
| 第二阶段 | 面向对象与视觉算法         | 能用类组织代码，用 OpenCV 传统方法识别装甲板   | 实现**相机类体系**（抽象基类 + 派生 + 工厂） | [`tutorial_2`](tutorial_2/tutotial_2.md) |
...
```mermaid
flowchart LR
    A["第一阶段<br/>开发环境与工具链<br/>（写代码）"] --> B["第二阶段<br/>面向对象 + OpenCV<br/>（写算法）"]
...
**培训目标**：学会用类把「数据 + 行为」封装成可复用的组件（抽象基类 + 多态 + 工厂），并用 OpenCV 传统图像处理从图像里稳定找出装甲板的四个角点。
