...
- [SPR 2027 算法组培训计划](#spr-2027-算法组培训计划)
  ...
  - [第一阶段：开发环境与工具链](#第一阶段开发环境与工具链)
    ...
    - [作业](#作业)
    - [教案与实例](#教案与实例)
  - [第二阶段：视觉算法与面向对象](#第二阶段视觉算法与面向对象)
    - [知识模块](#知识模块-1)
    - [作业](#作业-1)
    - [教案与实例](#教案与实例-1)
  - [第三阶段：ROS2 工程化与自瞄的数理逻辑](#第三阶段ros2-工程化与自瞄的数理逻辑)
    - [知识模块](#知识模块-2)
    - [作业](#作业-2)
    - [教案](#教案)
...
| 第二阶段 | 视觉算法与面向对象         | 用 OpenCV 传统方法识别装甲板，能用类组织代码   | 实现**相机类体系**（抽象基类 + 派生 + 工厂） | [`tutorial_2`](tutorial_2/tutotial_2.md) |
| 第三阶段 | ROS2 工程化与自瞄的数理逻辑 | 能把算法包成 ROS2 节点，并从像素反推目标位姿 | 实现**装甲板识别 ROS2 节点** + 位姿解算 | [`tutorial_3`](tutorial_3/tutorial_3.md) |
...
```mermaid
flowchart LR
    A["第一阶段<br/>开发环境与工具链<br/>（写代码）"] --> B["第二阶段<br/>OpenCV + 面向对象<br/>（写算法）"]
    B --> C["第三阶段<br/>ROS2 工程化 + 自瞄数理逻辑<br/>（上车算明白）"]
...
**培训目标**：学会用 OpenCV 传统图像处理从图像里稳定找出装甲板的四个角点，并用类把「数据 + 行为」封装成可复用的组件（抽象基类 + 多态 + 工厂）。
...
> 把「相机」抽象成一个类体系：抽象基类 `Camera` + `UsbCamera` / `IndustrialCamera` 两个派生类 + 工厂，再写一段只认基类引用的调用代码——换相机时算法一行不用改。
...
## 第三阶段：ROS2 工程化与自瞄的数理逻辑

**培训目标**：先把算法包成 ROS2 节点跑起来（能跑视频、也能直接接仿真），再打通「像素 → 角度」这条链路——能从图像中的四点反推目标的三维位姿，并算出云台该转到哪里、什么时候开火。

### 知识模块

| 模块                | 内容                                                                           |
| ------------------- | ------------------------------------------------------------------------------ |
| ROS2 通讯与工程结构 | 节点 / 话题 / 服务 / 参数；`ament_cmake` 功能包；`rosidl` 自定义消息；`cv_bridge` |
...
> 第一部分：把装甲板识别做成 ROS2 节点（订阅 `/image_raw` → 识别 → 发布**四个角点**），用**视频流**测试，随时能切到 Daedalus 仿真；第二部分：在识别结果的基础上完成**目标位姿解算与弹道解算**，输出可直接驱动云台的指令。

完整要求见 [`tutorial_3/homework_3.md`](tutorial_3/homework_3.md)。

### 教案

- [`tutorial_3/tutorial_3.md`](tutorial_3/tutorial_3.md)（ROS2 工程化一节已就绪，数理逻辑部分施工中）
...
> 也就是说：**你的系统订阅 `/image_raw` 等话题，发布 `/rm_gimbal/cmd`**。这正好是第三阶段两部分「装甲板识别节点 + 弹道解算」的组合。
>
> 模拟器 README 里写的 `/armor_solver/cmd_gimbal` 是**旧名字**，以代码为准（`src/ros2/topic.rs` 里是 `/rm_gimbal/cmd`，QoS `sensor_data`）。接口包是模拟器自带的 `rm_interfaces`（16 个 msg + 2 个 srv），第三阶段的消息都定义在里面。
...
│       ├── gpp/                 g++ 编译示例（单文件 / 多文件）
│       └── cmake/               CMake 最小构建示例
├── tutorial_2/                  第二阶段：视觉算法与面向对象
│   ├── tutotial_2.md            教案
│   ├── homework_2.md            阶段作业：相机类的封装与派生
│   └── example/
│       └── opencv/              装甲板四点识别
└── tutorial_3/                  第三阶段：ROS2 工程化与自瞄数理逻辑
    ├── tutorial_3.md            教案
    └── homework_3.md            阶段作业：ROS2 装甲板识别节点 + 位姿/弹道解算
```
